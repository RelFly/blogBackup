...
你有没有试过让 ChatGPT 直接调用你们公司的内部 API？大概率会碰壁——模型不知道你们的接口长什么样，不懂业务规则，甚至会编造不存在的参数。这就是 **Skill（技能）+ MCP（Model Context Protocol）** 要解决的问题组合。
...
正文部分是真正的"操作手册"。推荐采用统一的 **条件-动作-结果** 三段式格式：
...
- **条件**：用户表达新建/添加意图，例如：
  ...
  - "提醒我明天开会"
- **前置检查**：
  1. 标题（title）：必须提供。若缺失，反问用户"你想给这个待办起什么名？"
  2. 截止日期（due_date）：可选，但建议主动询问
  3. 优先级（priority）：可选，默认为 "medium"
- **动作**：调用 MCP 工具 `create_todo`，参数如下：
  ```
  title: 用户提供的标题
  due_date: ISO 8601 格式，如 "2026-05-10T09:00:00+08:00"
  priority: "low" | "medium" | "high"，默认 "medium"
  ```
- **结果处理**：
  - 成功：输出"已创建待办：{title}，ID: {id}，截止日期：{due_date}"
  - 失败：将错误信息转述给用户，并给出修正建议
...
- **条件**：用户表达查询意图，例如：
  ...
  - "我的待办列表"
  - "看看还有啥没做完"
- **动作**：调用 MCP 工具 `list_todos`：
  - 默认参数：`status=active`, `limit=5`
  - 若用户指定了筛选条件（如"高优的"、"本周的"），映射为对应参数
- **结果处理**：
  - 将返回的 JSON 渲染为 Markdown 表格：
  ```
  | ID | 标题             | 优先级 | 截止日期     | 状态   |
  |----|------------------|--------|-------------|--------|
  | 1  | 完成PPT          | 高     | 2026-05-10  | 进行中 |
  | 2  | 代码评审         | 中     | 2026-05-08  | 进行中 |
  ```
  - 超过 5 条时追加提示："仅展示前 5 条，如需更多请告知"

### 场景 C：修改待办

- **条件**：用户表达修改/变更意图
- **动作流程**：
  1. 调用 `list_todos` 获取当前列表（帮助定位目标）
  2. 向用户确认要修改哪一条（通过 ID 或标题）
  3. 询问新的参数值
...
- **强制流程（必须严格执行）**：
  ...
  2. 请用户确认目标："你要删除的是「{title}」(ID: {id}) 吗？"
  3. **获得用户的明确同意后**才调用 `delete_todo(id)`
  4. 如果用户说"全部删掉"，必须拒绝并解释："出于安全考虑不支持批量删除，请逐条确认"
- **结果**：告知用户"已删除待办：{title}"
```
...
| 3 | **枚举值必须穷举** | 不写清楚模型会自己编 | priority 只写了 high/low，没写 medium |
| 4 | **不要在 SOP 里重复写 Schema** | MCP 握手时已暴露 InputSchema | 在 references 里再抄一遍 JSON Schema |
...
这段响应就是大模型理解"我能做什么"的唯一来源。Schema 写得越精准，模型调用越靠谱。
...
async function handleListTodos(params) {
  const limit = Math.min(params.limit || 5, 20);  // 硬顶 20 条
...
| **SKILL.md 写法** | 元数据定义触发条件，正文用条件-动作-结果三段式写 SOP |
